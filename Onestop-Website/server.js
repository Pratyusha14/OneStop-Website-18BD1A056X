...
app.post('/AddData', (req,res)=>{
    db.collection('female').save(req.body,(err,result)=>{
      if(err) return console.log(err)
      res.redirect('/')
    })
})
...
app.post('/editupdate',(req,res)=>{
	...
	db.collection('female').find().toArray((err,result)=>{
		...
		db.collection('female').updateOne(id,newValue,(err,result)=>{
			if(err) return console.log(err);
			if(parseInt(req.body.pquantity)+parseInt(oldQuantity)<parseInt(oldQuantity)){
				db.collection('BooksSales').find({pid:req.body.pid}).toArray((err,da)=>{
					var flag=0;
...
app.post('/delete', (req,res)=>{
    db.collection('female').findOneAndDelete({pid: req.body.pid}, (err,result)=>{
      if(err) return console.log(err)
      res.redirect('/')
    })
})
...
app.post('/salesUpdate',(req,res)=>{
	db.collection('BooksSales').find({
		...
		db.collection('BooksSales').updateOne(que,query,(err,result)=>{
			if(err) return console.log("err");
			res.redirect('/sales')
		})
	})
